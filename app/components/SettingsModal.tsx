...
export function SettingsModal({
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 pt-[max(1rem,env(safe-area-inset-top))]" style={{ background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(24px)' }}>
      <motion.div
...
function SyncTab() {
  const [activeCode, setActiveCode] = useState<{ code: string; expiresAt: string } | null>(getActiveSyncToken());
  const [secondsLeft, setSecondsLeft] = useState(0);
  const [pullCode, setPullCode] = useState('');
  const [status, setStatus] = useState<'idle' | 'generating' | 'pulling' | 'success' | 'error'>('idle');
  const [message, setMessage] = useState('');
  const [copied, setCopied] = useState(false);

  // Countdown timer
  useEffect(() => {
    if (!activeCode) { setSecondsLeft(0); return; }
    const calc = () => Math.max(0, Math.ceil((new Date(activeCode.expiresAt).getTime() - Date.now()) / 1000));
    setSecondsLeft(calc());
  ...
  const handleGenerate = async () => {
    ...
    setMessage('');
    const { token, expiresAt, error } = await createSyncToken('My Device');
    if (error) { setStatus('error'); setMessage(error); return; }
    setActiveCode({ code: token, expiresAt });
    setStatus('idle');
    setMessage('');
  };
  ...
  const handlePull = async () => {
    ...
    setStatus('pulling');
    setMessage('');
    const { success, error } = await pullSyncData(pullCode);
  ...
  const copyCode = () => {
    ...
    navigator.clipboard.writeText(activeCode.code).catch(() => {});
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  };

  // Split the 6-digit code for display
